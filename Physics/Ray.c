...
bool RayCast(vec3 pos, vec3 forward, float distance, float increaseValue,
             vec3 dest)
{
    ...
    int lastBlock[] = { RoundPos(position[0]), RoundPos(position[1]),
                        RoundPos(position[2]) };
    ...
    {
        float height =
            GetSingleNoiseVal(position[0], position[2], NULL, heightNoise)
            * ChunkHeight / 2;

        int actualBlock[] = { RoundPos(position[0]), RoundPos(position[1]),
                              RoundPos(position[2]) };
        dest[0] = RoundPos(position[0]);
        dest[1] = RoundPos(position[1]);
        dest[2] = RoundPos(position[2]);

        if (height + 1 >= position[1] && !SameBlock(actualBlock, lastBlock)
            && DestroyPossible(actualBlock))
        {
            for (int i = 0; i < 3; i++)
            {
