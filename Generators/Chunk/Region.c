...
void InitRegion()
{
    for (int x = 0; x < RegionCount; x++)
    {
        ...
        {
            struct regionList *region =
                (struct regionList *)malloc(sizeof(struct regionList));
            region->x = x;
            region->z = z;
...
{
    int renderIndex = (Modulo((int)pos[0], ChunkSize) * ChunkSize
                       + Modulo((int)pos[2], ChunkSize))
            * ChunkHeight
        + (int)pos[1] + ChunkHeight / 2;
    ...
    int x = (int)RoundRegion((float)((int)pos[0] % chunkRegion)
                             / (ChunkSize + xAdd));
    int z = (int)RoundRegion((float)((int)pos[2] % chunkRegion)
                             / (ChunkSize + zAdd));

    int chunkIndex = (x + ChunkView) * ChunkView * 2 + z + ChunkView;

    struct regionList *actualRegion =
        regions[(int)position[0] + 1][(int)position[1] + 1];
...
{
    int renderIndex =
        (Modulo(pos[0], ChunkSize) * ChunkSize + Modulo(pos[2], ChunkSize))
            * ChunkHeight
        + pos[1] + ChunkHeight / 2;
    ...
    int x =
        (int)RoundRegion((float)(pos[0] % chunkRegion) / (ChunkSize + xAdd));
    int z =
        (int)RoundRegion((float)(pos[2] % chunkRegion) / (ChunkSize + zAdd));
